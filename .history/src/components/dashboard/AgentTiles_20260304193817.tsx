  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {agents.map(({ href, label, description, icon: Icon, gradient, iconColor, badge }) => (
          <Link
            key={href}
            href={href}
            className={cn(
          ...
          >
            ...
            <ArrowRight className="relative mt-1 h-4 w-4 shrink-0 text-[var(--text-dim)] opacity-0 transition group-hover:opacity-100 group-hover:translate-x-0.5" aria-hidden />
          </Link>
        ))}
      </div>
    </div>
